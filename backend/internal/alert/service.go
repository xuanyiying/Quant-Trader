...
func (s *AlertService) triggerAlert(a AlertCondition, candle model.KLine) {
	...
	// 1. Publish to a notification topic
	...
	// 2. Telegram notification (Enterprise feature)
	...
	// 3. Mark alert as inactive to prevent spamming
	_, err := s.db.Exec(context.Background(), "UPDATE alerts SET is_active = FALSE WHERE id = $1", a.ID)
	if err != nil {
		s.logger.Error("failed to deactivate triggered alert", zap.Error(err), zap.Int64("alert_id", a.ID))
	}
}
